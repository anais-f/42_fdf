...
/*   Updated: 2024/03/05 13:12:53 by anfichet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "fdf.h"
...
void	bresenham_low(t_topo topo_start, t_topo topo_end, t_data *data)
{
	...
	while (topo_start.px <= topo_end.px && ((topo_start.px >= 0 && topo_start.px
				<= WIDTH) || (topo_start.py >= 0 && topo_start.py <= HEIGH)))
	{
		my_mlx_pixel_put(data, topo_start.px, topo_start.py, \
					data->color[data->actual_color]);
		if (param.decision > 0)
		{
...
void	bresenham_high(t_topo topo_start, t_topo topo_end, t_data *data)
{
	...
	while (topo_start.py <= topo_end.py && ((topo_start.py >= 0 && topo_start.py
				<= HEIGH) || (topo_start.px >= 0 && topo_start.px <= WIDTH)))
	{
		my_mlx_pixel_put(data, topo_start.px, topo_start.py, \
						data->color[data->actual_color]);
		if (param.decision > 0)
